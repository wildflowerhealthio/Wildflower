import { type Error as PlatformError, type FileSystem } from '@effect/platform'
import { Effect } from 'effect'
import { File } from 'expo-file-system'
import * as Telemetry from '../../telemetry.ts'
import { fileUri, instrument, systemError, trySystem } from './helpers.ts'
...
const writeFile: FileSystem.FileSystem['writeFile'] = (path, data, options) => {
  ...
  const effect: Effect.Effect<void, PlatformError.SystemError> =
    options?.flag !== undefined && options.flag !== 'w'
      ? Effect.fail(
          systemError(
            'writeFile',
            path,
  ...
  return effect.pipe(instrument('writeFile', Telemetry.FileSystem.WriteFile.Span.Name, path))
}
