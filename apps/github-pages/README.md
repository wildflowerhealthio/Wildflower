...
untouched — in particular the medications app and the Web Trace app keep
building into `slices/apps/self-hosted-apps/medication` and
`slices/apps/self-hosted-apps/web-trace`, where they are also vendored as Tauri
resources. Those paths are each a seeded app row's `content_folder`, so this
package only copies from them rather than redirecting them. The server-docs
console builds into its own `dist/` and is copied verbatim.
...
`launch.html` SMART launch entry of the medications and Web Trace apps) is
missing — a silently empty upstream build never gets published. Running it on its own expects the
