...
interface AppShellWebViewProps {
  /**
   * Fires every time the SPA emits `RouteChanged`. Optional — the host no
   * longer consumes route changes (top-level navigation lives in the SPA's
   * own tab bar), so it defaults to a no-op.
   */
  readonly onRouteChanged?: (event: { pathname: string; canGoBack: boolean }) => void
}

// The SPA's top-level surfaces stay inside the WebView; any other in-origin
// path is treated as a deep link and opens in the system browser. Kept
// local so the host's routing decision doesn't depend on the SPA-only tab
// spec.
const APP_ROUTE_PREFIXES = ['/home', '/collector', '/settings'] as const
...
const makeShouldOpenInSystemBrowser =
  ...
  (urlString: string): boolean => {
    try {
      ...
      if (isRootPath || isAppRoute(url.pathname)) return false
...
const AppShellWebView = ({
  onRouteChanged = noopRouteChanged,
