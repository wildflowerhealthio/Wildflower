...
import { LOCAL_HOSTNAME } from '../daemons/http-server.ts'
...
function useShellOrigins(): ShellOrigins {
  ...
  const publicHostname =
    tunnelState.currentSubdomain !== null && tunnelState.currentRootDomain !== null
      ? `${tunnelState.currentSubdomain}.${tunnelState.currentRootDomain}`
      : null
  return {
    store,
    running: serverState.running,
    localHostname: serverState.localHostname ?? LOCAL_HOSTNAME,
    publicHostname,
  }
}
