...
/**
 * Pins the `runAuthed` runner this PR threads into the TanStack Router
 * {@link RouterContext}.
 *
...
describe('runAuthed router-context runner', () => {
  ...
  // Type-level guard: the `RouterContext` this PR extends carries a
  // `runAuthed` of the `RunAuthed` shape. A future edit that drops the
  // field or changes its requirement type fails to compile here, which
  // `vp check` surfaces. (`satisfies` keeps the literal honest without an
  // unsafe cast; the runner body is never invoked.)
  it('should type RouterContext.runAuthed as a RunAuthed', () => {
    // Arrange / Act
    const tokenRef = Effect.runSync(SubscriptionRef.make<string | null>(null))
    const { runAuthed, dispose } = makeRunner(tokenRef)
    disposers.push(dispose)
