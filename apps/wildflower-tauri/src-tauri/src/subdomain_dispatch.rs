...
//! internal app's static file router when the request's `Forwarded` host
...
//! forwarded request's `Forwarded` host is exactly
...
//! ## Match strictness
//!
//! The match is `forwarded-host == "<app-id>.<configured-public-host>"`
...
/// `Forwarded` host matches `<internal-app-id>.<configured-public-host>`,
...
pub(crate) async fn maybe_dispatch_to_internal_subdomain(
    ...
    let Some(app_id) =
        match_internal_subdomain(host, &public_host, |id| state.routers.contains_key(id))
    else {
        return next.run(req).await;
    };
    let router = state
...
mod tests {
    ...
    /// A forwarded request whose `Forwarded` host matches
    ...
    async fn forwarded_request_to_the_apex_host_falls_through_to_the_api() {
        ...
        let res = router
            .oneshot(request(&[(
                "forwarded",
                "host=demo.example.com;proto=https",
            )]))
            .await
            .expect("oneshot");
    ...
    /// A direct loopback request (no `Forwarded` header) ALWAYS goes to the
    ...
    fn match_internal_subdomain_handles_edge_cases() {
        ...
        assert_eq!(
            match_internal_subdomain("apex", "demo.example.com", is_known),
            None
        );
