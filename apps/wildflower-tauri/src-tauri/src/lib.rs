...
use crate::subdomain_dispatch::{
    maybe_dispatch_to_internal_subdomain, SubdomainDispatchState,
};
...
async fn run_server(
    ...
    // per-app socket, so a request whose `x-public-origin` is
    ...
    let router = Router::new()
        ...
        // forwarded request whose `x-public-origin` matches
