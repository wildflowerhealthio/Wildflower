...
use gatekeeper_rust::{layer_router_with_gatekeeper_auth_gating, setup_gatekeeper, GatekeeperConfig};
...
async fn run_server(runtime: ServerRuntimeConfig) -> anyhow::Result<()> {
    ...
    let gatekeeper = setup_gatekeeper(&gatekeeper_config, &token_origin)
    ...
    let token = &gatekeeper.host_owner_token;
