...
"Medications (Dev)" tile that launches whatever is serving
that port — the vite dev server when it is up, otherwise the
host's copy of the vendored build. The port is pinned in both `vite.config.ts`
and the dev seed; changing it means changing both.
