...
type Component = 'read' | 'write'

/** The components in display order. */
const COMPONENTS: readonly Component[] = ['read', 'write']

/** User-facing label for each component (1:1 with the SMART v1 word). */
const LABEL: Readonly<Record<Component, string>> = {
  read: 'Read',
  write: 'Write',
}
...
const covers = (access: AccessRights.AccessRights | null, component: Component): boolean => {
...
const of = (access: AccessRights.AccessRights | null): Readonly<Record<Component, boolean>> => ({
...
export { type Component, COMPONENTS, LABEL, covers, of, toAccess }
