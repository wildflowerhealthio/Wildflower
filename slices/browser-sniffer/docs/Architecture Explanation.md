...
5. Accepts host→web `Click` and `CancelSnifferRequest` messages via direct `eventBus.listen('bridge:Click' | 'bridge:CancelSnifferRequest', …)` registrations. No `window.message` indirection — Tauri's IPC is the only thing that can invoke these listeners, so page scripts can't spoof inbound messages.
...
- **Effect-Messaging Web side**: The sniffer runs in _arbitrary_ third-party pages — it can't depend on our SPA bundle's `WebPlatformAdapter`. So it speaks the wire format manually (`void eventBus.emit('bridge:{tag}', msg)`) and the host decodes it through `BridgeTransport`'s `Host` side only.
