...
- **`browser-sniffer-injected`** — the actual JS that goes into a third-party page. Real TypeScript source (`src/install-sniffer.ts`); a string export (`snifferScript`) the platform-adapter packages embed verbatim into the host webview's pre-content initialization script.
- **`browser-sniffer-tauri`** — TypeScript bootstrap that bridges the sniffer's `window.ReactNativeWebView.postMessage` outbound channel onto Tauri's event bus, wraps `installSniffer()` for injection via `WebviewWindowBuilder::initialization_script`, and injects an in-page `BrowserTopBar` for perceived separation on iOS. Bundles to a self-contained IIFE (`dist/tauri-bootstrap.js`, regenerated by `pnpm install`) which the Rust crate includes via `include_str!`. See [Tauri Host Explanation](./Tauri%20Host%20Explanation.md) for the full plumbing.
...
## Why a build-time bundle (not a Vite plugin or runtime stringification)

`installSniffer` is esbuild-bundled to a self-contained IIFE at build time by `scripts/build-sniffer-script.mjs`, written into a committed generated file, and re-exported as the `snifferScript` string. The host adapter embeds those bytes verbatim into the webview's initialization script.
