...
| Tag                                   | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RequestSniffableWebView`             | Present the sniffer's native webview **on `about:blank`** (the payload carries no starting page): `native_webview().open_url("about:blank", ...)` (navigate, building hidden if absent) then `native_webview().show()`. A second tag while it is up rebinds the existing native webview in place (channel + initScript + chrome + URL — see "Re-wire" below). The plan drives the first real navigation with a leading `Open` step.                |
| `Open`                                | Decode the `Open` payload, resolve its `WebViewSource`, then `open_or_navigate` — like `RequestSniffableWebView`'s mount but to a real URL (opened fresh if absent, else navigated in place). This is how a plan navigates off `about:blank` and every step thereafter. Distinct tag/payload so the SPA can drive a navigation independently of the initial present.                                                                               |
