...
| `PageAction` / `QueryMatches` / `CancelSnifferRequest` | **Mobile only**: forwarded into the native webview via `native_webview().evaluate_js(...)`. Desktop's native-webview content webview is still a Tauri webview (`__TAURI__.event.listen`) and receives Rust `app.emit('bridge', …)` directly. `PageAction` carries the scripted interaction (`Click` / `Fill`, demuxed page-side by `action.kind`); `QueryMatches` asks the page to enumerate the DOM (answered by a `MatchesFound` on the data plane) for the collector's `ForEach` link discovery; the host forwards both by `_tag` without decoding. |

The data plane is host-mediated on **both** platforms: the content webview loads untrusted third-party content, so its web→host `bridge:ResponseStart` / `ResponseData` / `PageLoaded` / `MatchesFound` / etc. never reach `BRIDGE_EVENT` straight from the page — the host allowlists the inner `_tag` (the data-plane set, excluding control tags) and re-broadcasts. Two transports, one gate:
...
- **Desktop**: a Tauri parent `Window` with two child webviews via `Window::add_child` — a chrome bar on top and the external URL underneath. The content webview is still a Tauri webview (so `__TAURI__` is present), scoped by `apps/wildflower-tauri/src-tauri/capabilities/native-webview-window.json` to event-bus `listen` (inbound `PageAction` / `QueryMatches` / `CancelSnifferRequest`) plus the gated `native_webview_data_plane_emit` command — see below.
...
Both variants call `installSniffer(event)`, which wires fetch / XHR / console shims and registers a single `event.listen(BRIDGE_EVENT, …)` that demuxes inbound `PageAction` / `QueryMatches` / `CancelSnifferRequest` by the payload's `_tag` (and `PageAction` further by its inner `action.kind`; `QueryMatches` answers with a `MatchesFound`). **No in-page `BrowserTopBar`** — the plugin draws native chrome on all three platforms, so the in-page bar is gone for good.
...
- **`hide`** — a user dismissal (desktop titlebar X, iOS Close/swipe, Android Toolbar/back) removes it from view but keeps it **alive and running**. It emits `NativeWebviewEvent::Hidden`, which `native_webview_bridge::dispatch_body` treats as non-terminal (just logs): the sniff continues in the background. `SniffingComplete` is **SPA-driven**, so a hide never ends the sniff.
- **`dispose`** — the SPA's `SniffingComplete` is the terminal signal; `sniffing_complete::handle` calls `native_webview().dispose()`, which tears the webview down and frees resources, emitting `NativeWebviewEvent::Disposed` (lifecycle-only on the host side — the SPA already observed `SniffingComplete`). A teardown backstop also disposes: app teardown on every platform, plus a 5-minute hidden-idle timeout on mobile.
