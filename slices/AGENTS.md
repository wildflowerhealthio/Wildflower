...
Current slices: `apps`, `collector`, `gatekeeper`, `store`, `telemetry`.
...
- **Compose `HttpApi` groups across slices via the phantom-id bridge pattern** — see the [Learnings Inbox](../docs/Agents/Learnings%20Inbox.md) entry on `*ApiHandlersFor<ParentId>()`
- **Don't use `topLevel: true` on multiple `HttpApiGroup`s under the same `HttpApi`** — name collision in the generated client. See Learnings Inbox.

## References

- [Effect Patterns Reference](../docs/Effect/Patterns%20Reference.md) — Tag/Layer wiring used inside slice cores
- [Learnings Inbox](../docs/Agents/Learnings%20Inbox.md) — Slice-specific HttpApi/LiveStore/vp-pack gotchas
