...
Current slices: `apps`, `branding`, `browser-sniffer`, `collector`, `databases`, `emr`, `gatekeeper`, `http-extraction`, `importer`, `medication-interaction`, `medication-matching`, `medication-sponsorship`, `navigation`, `persistence`, `scopes`, `shared-structures`, `telemetry`, `tunnel`, `web-trace`. Verify with `ls slices/` — this list can go stale.
...
`medication-matching` owns the fundamentals of matching a patient's medication names against a drug catalog (the minimal `Medication` type, name normalization, containment scoring) — see [medication-matching/AGENTS.md](./medication-matching/AGENTS.md). Two slices build on it and neither owns it: [`medication-sponsorship`](./medication-sponsorship/AGENTS.md) matches against patient-support program lists, and [`medication-interaction`](./medication-interaction/AGENTS.md) against the DDInter drug-interaction database.
