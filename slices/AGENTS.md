...
Current slices: `apps`, `branding`, `browser-sniffer`, `collector`, `databases`, `emr`, `gatekeeper`, `http-extraction`, `importer`, `medication`, `navigation`, `persistence`, `scopes`, `shared-structures`, `telemetry`, `tunnel`, `web-trace`. Verify with `ls slices/` — this list can go stale.
