...
- **`date` is equality at one instant — and therefore near-unusable as declared.** FHIR's date prefixes (`gt`/`lt`/`ge`/`le`/`sa`/`eb`/`ap`) and partial-precision ranges are not expressible; a partial-precision wire value like `date=2026-07` decodes to the instant it names (`2026-07-01T00:00:00.000Z`) rather than the July-2026 range FHIR reads it as. This is **stricter** than Patient's `birthdate`, which stays a `YYYY-MM-DD` string and so keeps FHIR's whole-day range semantics: `DateTime.Utc` always encodes full millisecond precision, and `DocumentReference.date` is an `instant`, so only a caller who already knows a document's exact millisecond can match it. "Documents dated 2026-07-27" sends `date=2026-07-27T00:00:00.000Z` and comes back as an empty `Bundle`. Modelling the parameter as a passthrough FHIR-date string (with the prefix grammar), as `birthdate` does, would fix it.
