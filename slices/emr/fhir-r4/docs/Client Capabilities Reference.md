...
- **Related resources are the patient-referencing types in the `RELATED_RESOURCE_TYPES` table — `Observation` and `MedicationRequest` today.** FHIR `$everything` returns every resource in the patient's compartment; we include only the types in that table (matched on their `subject` reference). This is _broader_ than the TypeScript twin, which relates `Observation` only because its LiveStore models just Patient/Observation/Binary — HFS is a general FHIR store, so the Rust side gathers every patient-referencing type it serves. Add a row to the table (and this list) to include another type.
- **Related-resource matching is in-memory over one search page, per type.** HFS's `subject` search parameters aren't indexed (the embedding opens with `data_dir: None`), so instead of a compartment/`subject=` search we fetch each type's type-level search (each capped at HFS's `max_page_size`, 1000) and filter `subject.reference == "Patient/{id}"` in memory. Only the first page per type is inspected, so once the store holds more than 1000 of a type across **all** patients, a target patient's rows outside that page are silently dropped — even a patient with only a few. This is stricter than the TypeScript twin, which queried the store unbounded. Closing it needs the delegated search paged to exhaustion, or a real `subject`-indexed compartment search. A related-type search that errors (non-200 or an unreadable body) is logged and treated as no matches, so a partial-scope token still gets the Patient and the types it can read.

Additionally, `_count` truncates the combined matched related set and `Bundle.total` reflects the returned (post-truncation) entry count rather than the grand match total.
