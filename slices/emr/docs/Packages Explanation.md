...
- **`fhir-r4-react`** — React adapter: the tokenless client runtime layer (`FhirR4ResourcesRouterContext`) and query hooks (`usePatientsQuery`). Documented cross-slice consumers: `collector-react`'s sync runner and `gatekeeper-react`'s consent-screen patient picker. Its `./smart` entry is the other half — the SMART App Launch handshake (`authorizeSmartLaunch` / `readySmartClient`) and the **self-hosted runtime** (`buildSmartRouterContext`) a SMART app served from its own origin wires itself up with. Consumer today: `apps/web-trace`; it lives here rather than in that app because the wiring is a property of self-hosting, not of any one app.
