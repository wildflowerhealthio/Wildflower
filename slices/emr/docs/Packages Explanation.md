...
- **`fhir-r4-react`** — React adapter: the tokenless client runtime layer (`FhirR4ResourcesRouterContext`) and query hooks (`usePatientsQuery`). Documented cross-slice consumers: `collector-react`'s sync runner and `gatekeeper-react`'s consent-screen patient picker. Its `./smart` entry is the other half — the SMART App Launch handshake (`authorizeSmartLaunch` / `readySmartClient`), the standalone-launch primitives (`detectSmartSupport` / `startStandaloneLaunch`), and the **self-hosted runtime** (`buildSmartRouterContext`) a SMART app served from its own origin wires itself up with. Its `./connect` entry is the standalone connect menu (`ConnectMenu`) an app mounts so a user can pick a FHIR server and launch against it — a react-tundraish UI, which is why this package now depends on the design system. Consumers today: `apps/web-trace`, `apps/medications-app`; the wiring lives here rather than in those apps because it is a property of self-hosting, not of any one app.
