...
- **`fhir-r4`** — standalone FHIR R4 package: pure Effect schemas for datatypes and resources (`data-types/`, `resources/`), the `HttpApi` description of the FHIR REST surface (`http-api-definition/`, prefix `/fhir-r4`), the derived HTTP client and batch write sink (`clients/`), and the source-id derivation every collector re-keys with (`identity/`). No server implementation, no persistence — the decoded type of every schema is a plain FHIR R4 value.
