-- Seed the medication-sponsorship self-hosted SMART app. Schema comes from
...
-- gatekeeper migration 0004_seed_medication_sponsorship_client) so `is_smart`
-- is true, and a `launch_path` that starts the SMART EHR launch off its own
-- origin. Its build is vendored under
-- `slices/apps/self-hosted-apps/medication-sponsorship/` and synced into
...
INSERT INTO app_registrations (id, kind, position, on_homescreen, name, subtitle, local_only, client_id, requires_tunnel) VALUES
    ('medication-sponsorship', 'self-hosted', 6, 1, 'Sponsored Medications', 'See which of your medications have sponsorship coverage.', 0, 'medication-sponsorship', 0);

INSERT INTO self_hosted_app_configurations (id, port, content_folder, subdomain, seeded, launch_path) VALUES
    ('medication-sponsorship', 8090, 'medication-sponsorship', 'medication-sponsorship', 1, '/launch.html?launch={launch}&iss={origin}/fhir-r4');
