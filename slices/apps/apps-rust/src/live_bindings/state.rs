...
//! (plus the loopback base URL the non-tunnel launch origin + self-hosted hostname
//! derive from, the tunnel-launch resolver, the on-device webview seam, and the
//! host-seam ports: launch cookies, per-app launch scopes).
//!
//! Lives at the crate root (not under [`crate::http`]) deliberately: the
...
//! [`live_bindings`](super) module — which name the concrete [`SqliteAppsStore`]
//! adapter and `build` a capability from this state — while the generic capability
//! structs in `domain/` stay store-agnostic and free of any `crate::http` import
//! (gatekeeper/databases style; see
