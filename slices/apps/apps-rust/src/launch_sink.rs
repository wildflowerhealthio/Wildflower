...
//! [`LaunchSink::open`] demands a [`LoopbackCaller`] witness, and the only way
//! to mint one is [`LoopbackCaller::from_provenance`] on a
...
/// which is the precondition [`LaunchSink::open`] requires.
...
/// Host seam for the launch side-effect. The platform-pure handler resolves the
/// URL, then hands it (with its app) to the host's sink instead of returning a
/// redirect. Fire-and-forget: the handler 204s and the host logs any failure
/// (mirrors the SPA's prior fire-and-forget bridge emit).
///
