//! The [`AppsCreator`] capability — the `wildflower/Apps.c` door to registering a
//! new cloud (`POST /cloud-apps`) or self-hosted (`POST /self-hosted-apps`) app.
//! Holds its `*_scopes()` mapping (read by both its binding and
//! [`grantable_apps_scopes`](super::grantable_apps_scopes) so enforced and
//! grantable can't drift) and its store-focused tests.
...
/// Register a new cloud / self-hosted app — `wildflower/Apps.c`. Shared by the
