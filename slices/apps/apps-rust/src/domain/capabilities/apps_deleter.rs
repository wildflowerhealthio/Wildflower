...
mod tests {
    ...
    #[test]
    // The single-threaded fake installer's `Arc` is non-Send/Sync only in test; the
    // production binding uses the `Send + Sync` service.
    ...
    fn deleter_self_hosted_teardown_brackets_the_store_delete() {
        ...
        deleter.delete_by_id("my-app").expect("delete self-hosted");
        assert_eq!(
