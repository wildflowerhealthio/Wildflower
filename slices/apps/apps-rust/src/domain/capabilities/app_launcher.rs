//! The [`AppLauncher`] capability — launch authorization for `GET` / `POST
//! /apps/{id}`. The **hybrid** flavour: its binding implements
//! [`Capability`](scope_capabilities_rust::Capability) directly rather than the
//! fixed-scope shortcut, so the builder can stash the caller's [`Grant`] for the
//! data-dependent per-app SMART check while the static `wildflower/launch` umbrella
//! is still enforced by the [`Scoped`](scope_capabilities_rust::Scoped) extractor
//! before `build`. Holds its `*_scopes()` mapping (read by both its binding and
//! [`grantable_apps_scopes`](super::grantable_apps_scopes) so enforced and
//! grantable can't drift) and its authorization test.
...
/// Shared by the capability's binding and
/// [`grantable_apps_scopes`](super::grantable_apps_scopes) so enforced and
...
/// [`Capability`](scope_capabilities_rust::Capability) directly: the static
/// umbrella `wildflower/launch` (from [`app_launcher_scopes`]) is enforced by the
/// [`Scoped`](scope_capabilities_rust::Scoped) extractor, **and** the builder
/// stores the caller's [`Grant`] for the data-dependent per-app SMART check
/// ([`missing_launch_scopes`](AppLauncher::missing_launch_scopes)).
///
