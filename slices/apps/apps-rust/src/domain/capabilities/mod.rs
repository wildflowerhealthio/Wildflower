...
//! [`AppsDeleter`], and the hybrid [`AppLauncher`] — each carrying its struct, its
//! `*_scopes()` required-scope function, its store logic, and its own tests. The
...
mod tests {
    ...
    /// `*_scopes()` function (colocated in that capability's file), and
    /// [`grantable_apps_scopes`]'s `declared` array must list all of them. A
    /// capability added without registering enforces a scope the grantable
    /// vocabulary never offers — a silent lock-out. Counting the scope functions by
    /// their crate-visible `app…`-prefixed signature across the capability files (the
    /// four `apps_*_scopes` + `app_launcher_scopes`; the grantable fn is `pub`, and no
    ...
    fn every_capability_scope_fn_is_registered_in_the_grantable_vocabulary() {
        const SOURCES: &[&str] = &[
            include_str!("apps_reader.rs"),
            include_str!("apps_creator.rs"),
            include_str!("apps_editor.rs"),
            include_str!("apps_deleter.rs"),
            include_str!("app_launcher.rs"),
        ];
        // Assembled from fragments so this needle's own literal here isn't miscounted
        // (mod.rs isn't among the scanned sources, but keep the guard regardless).
        let needle = concat!("pub(crate) fn ", "app");
        let scope_fns: usize = SOURCES
            .iter()
