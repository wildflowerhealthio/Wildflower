//! The [`AppsEditor`] capability — the `wildflower/Apps.u` door to content edits
//! (`PUT /cloud-apps/{id}`, `PUT /self-hosted-apps/{id}`) and home-screen
//! placement (`PUT /home-screen`). Holds its `*_scopes()` mapping (read by both its
//! binding and [`grantable_apps_scopes`](super::grantable_apps_scopes) so enforced
...
/// Edit an app's content / home-screen placement — `wildflower/Apps.u`. Shared by
