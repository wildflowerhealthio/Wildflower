//! Shared HTTP response templates for the apps handlers. The logged opaque-500
//! ([`InternalError`]) is the shared one from `shared-structures-rust` (the same
//! type the other `-rust` slices use); the `Result`-returning [`HandlerError`]
//! lets a fallible step bail with `?` instead of a `match` + `into_response`.
//!
//! On top of that shared shape the apps slice carries domain-level
