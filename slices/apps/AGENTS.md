...
- The committed `default.json5` config override is embedded via `include_str!` on the Rust side — config changes need the Rust build to pick them up.
...
- [self-hosted-apps README](./self-hosted-apps/README.md) — vendoring model, per-origin serving, config override
