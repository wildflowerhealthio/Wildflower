...
- **`self_hosted_apps` is no longer read-only.** The owner-gated `POST /apps` create endpoint is `multipart/form-data` discriminated on `provenance`: the self-hosted arm carries an uploaded `bundle` and inserts `apps` + `self_hosted_apps` rows (auto-suffixed slug, allocated port); `DELETE /apps/{id}` removes non-seeded ones. The migration-seeded rows carry `seeded = 1` and stay delete-protected (`409 AppNotEditable`); `AppListEntry.removable` is the client-facing removability contract.
