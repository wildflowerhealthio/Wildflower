...
/**
 * The LifeLabs collector's persist sink: the descriptor's `persistResources`.
 ...
 *
 * This is a verbatim copy of `fhir-r4-client-collector/src/persist.ts` (and of
 * `rexall-be-well-collector/src/persist.ts`): every FHIR-targeting collector
    ...
    (resource) => {
      ...
      return upsertResource(resource).pipe(
        ...
        Effect.tapError((cause) =>
          Effect.logError(`lifelabs persist: upsert failed for ${failed.label}/${failed.id}`, cause)
        ),
