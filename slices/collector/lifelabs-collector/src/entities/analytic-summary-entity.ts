...
/**
 * The provisional coding system for LifeLabs' internal test codes (the
 * `testCode` on each analytic, e.g. `TR10477-8W`). LifeLabs publishes no OID for
 * these, so this is a namespaced placeholder — reconcile against a real system
 * URL if one is confirmed. `code.text` (the human-readable analyte name) is the
 * load-bearing field; the coding is supplementary.
 */
const LIFELABS_TEST_SYSTEM = 'https://mycarecompass.lifelabs.com/test-code'
 ...
 * `/Date(ms±hhmm)/` token.
 */
const AnalyticSchema = Schema.Struct({
...
type Analytic = typeof AnalyticSchema.Type

/** One row of `entity.patients[]` — the account's patient identities. */
const PatientRowSchema = Schema.Struct({
 ...
 * three fields the entity reads (`analytics`, `selectedPatient`, `patients`) are
 * modeled; the rest of the payload (`testItems`, `visitDateRange`, status
...
const SummarySchema = Schema.Struct({
  entity: Schema.Struct({
    analytics: Schema.optionalWith(Schema.Array(AnalyticSchema), {
      default: (): readonly Analytic[] => [],
    }),
    selectedPatient: Schema.optional(Schema.NullOr(Schema.Union(Schema.String, Schema.Number))),
    patients: Schema.optionalWith(Schema.Array(PatientRowSchema), {
      default: (): readonly (typeof PatientRowSchema.Type)[] => [],
    }),
  }),
})

const decodeSummary = Schema.decode(Schema.parseJson(SummarySchema))
const decodePatient = Schema.decodeUnknown(Patient.Schema)
const decodeObservation = Schema.decodeUnknown(Observation.Schema)

/** `…://on-api.mycarecompass.lifelabs.com/api/Report/GetAnalyticSummary`. */
const analyticSummaryUrl = UrlMatch.make({
  segments: [
    UrlMatch.literal('api'),
    UrlMatch.literal('Report'),
    UrlMatch.literal('GetAnalyticSummary'),
  ],
})
 ...
 * `null` if unparseable. The trailing `±hhmm` is a display-only original offset;
 * the millis are already absolute UTC.
 */
const dotNetMillis = (raw: string): number | null => {
 ...
 * collide. Returns `null` when there is no code at all to key on (the caller
 * drops-and-counts those).
 */
const observationId = (a: Analytic): string | null => {
  const base = a.testItemId ?? a.testCode
  if (base == null || base.length === 0) return null
  const token = toFhirIdToken(base)
  const millis = a.collectionDate != null ? dotNetMillis(a.collectionDate) : null
  const id = millis != null ? `${token}-${millis}` : token
  return id.slice(0, 64)
}

/**
 * Build the R4 `Patient` **wire** object: `id` is the selected patient (the id
 * every Observation's `subject` references), and the display name (when present)
 * lands as a single `name[].text` entry (the source gives one combined string,
 * not structured family/given).
 */
const patientWire = (id: string, name: string | null): Record<string, unknown> => {
  const wire: Record<string, unknown> = { resourceType: 'Patient', id }
  if (name != null && name.length > 0) wire['name'] = [{ text: name }]
  return wire
}

/**
 * Build the R4 `Observation` **wire** object from one analytic. `status` is
 * `final` (these are posted results). `code.text` is the analyte name, with a
 * supplementary LifeLabs coding. A numeric result becomes a `valueQuantity`
 * (no unit is present in the source), anything else a `valueString`. The
 * reference range keeps its raw text plus parsed `low`/`high` when numeric, and
 * an `abnormalFlag` (when present) becomes an `interpretation`.
 */
 ...
 * `www.on.mycarecompass.lifelabs.com/analytics` loads. Unlike the Rexall
 * collector (which sniffs a FHIR STU3 Bundle), this response is a **bespoke,
 * non-FHIR JSON** shape, so it does not go through `fhir-stu3-as-r4`; the entity
 * decodes it and **synthesizes R4 resources**:
 *
 ...
 * dropped and counted via `Effect.logInfo` — the `ObservationListEntity`
 * drop-and-log pattern — so those losses aren't invisible. {@link extractJson}
 * normalizes the body across raw-XHR intercepts and the mobile WebView's
 * JSON-viewer wrap.
 */
const AnalyticSummaryEntity: EntityDefinition.EntityDefinition<FhirResource> =
  EntityDefinition.make({
    name: 'AnalyticSummaryEntity',
    isFoundAt: (url) => analyticSummaryUrl.test(url),
    parse: (response) =>
      Effect.gen(function* () {
        ...
        const subjectId = selected != null ? String(selected) : null
        ...
        if (subjectId != null) {
          ...
          const patient = yield* decodePatient(patientWire(subjectId, primary?.text ?? null))
        ...
        for (const a of analytics) {
          ...
          const observation = yield* decodeObservation(observationWire(a, subjectId, id))
          resources.push(observation)
        }
        if (dropped > 0) {
          yield* Effect.logInfo(
            `AnalyticSummaryEntity: dropped ${dropped} of ${analytics.length} analytics with no test code/item id to key an Observation`
          )
        }
