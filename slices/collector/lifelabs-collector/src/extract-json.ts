 ...
 * (`<` → `&lt;`, `&` → `&amp;`, etc.).
 *
 * The MyCareCompass SPA's `GetAnalyticSummary` payload arrives as an XHR
 * intercept (raw JSON, so the fast path returns it untouched); the `<pre>`
 * unwrap covers the fallback where the host snapshots a rendered JSON viewer
 * instead. This is a verbatim copy of `fhir-r4-client-collector`'s (and
 * `rexall-be-well-collector`'s) `extract-json.ts` — slice layering forbids one
 * `*-client-collector` importing another.
 *
 * Strategy:
 ...
 *     `application/json`; if the API ever serves `text/html` directly,
 *     the parse fails downstream and the surfaced error makes the
 *     boundary obvious.
 */
