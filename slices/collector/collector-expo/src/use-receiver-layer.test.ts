...
import type { useCollectorReceiverLayer } from './use-receiver-layer.ts'

// Type-only assertions on `useCollectorReceiverLayer`'s signature.
...
// holds.
expectTypeOf<ReturnType<typeof useCollectorReceiverLayer>>().toEqualTypeOf<
  Layer.Layer<MessageHandler.TagId<'Collector', 'Host'>>
>()
expectTypeOf<Parameters<typeof useCollectorReceiverLayer>>().toEqualTypeOf<[]>()

// Jest globbing requires at least one `describe`/`it` in any matched
// file — the assertions above carry the actual contract.
describe('useCollectorReceiverLayer (type-level)', () => {
  it('compiles', () => {
