...
/**
 * Covers the Shoppers Drug Mart persist sink — the descriptor's batch
 * `persistResources` (a verbatim copy of the FHIR R4 sink, so this mirrors
 * `fhir-r4-client-collector`'s persist test). Two concerns:
 *
 ...
 *   `HttpClient` — cast-free. (The `switch` itself now lives in `fhir-r4`'s
 *   `upsertResource`; this pins that a batch reaches it correctly.)
 * - **Failure accounting**: the sink owns retries, so a resource still failing
 *   after them comes back as a {@link CollectorDescriptor.PersistFailure} (with
 *   its cause) while the batch Effect itself never fails — one bad resource
 *   can't sink the run.
 *
...
describe('describeResource', () => {
  it('labels a resource by its FHIR resourceType and id', () => {
    const observation = byType('Observation').make('obs-1')
 ...
 * concrete schema — mirrors the persist switch so a missing arm shows up as a
 * missing case here.
 */
