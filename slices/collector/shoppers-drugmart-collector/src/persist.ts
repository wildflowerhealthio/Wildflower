...
/**
 * The Shoppers Drug Mart collector's persist sink: the descriptor's
 * `persistResources`. Writes a decoded batch back to the typed
 * {@link FhirR4ResourcesHttpApiClient} and hands back the resources it could
 * not write as {@link PersistFailure} data.
 *
 * This is a verbatim copy of `fhir-r4-client-collector/src/persist.ts` (also
 * copied into `rexall-be-well-collector`): every FHIR-targeting collector
 * writes to the same on-device FHIR R4 store through the same
 * {@link upsertResource} dispatch, so the sink is identical. Slice layering
 * forbids a `*-client-collector` importing another, so the file is duplicated
 * rather than shared (noted in the PR description).
 *
    ...
    (resource) => {
      ...
      return upsertResource(resource).pipe(
        ...
        Effect.tapError((cause) =>
          Effect.logError(
            `shoppers-drugmart persist: upsert failed for ${failed.label}/${failed.id}`,
            cause
          )
        ),
