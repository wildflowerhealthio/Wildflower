...
//!  - [`db`] — the SQLite store ([`db::RemotesStore`]) built on the shared
//!    `persistence-rust` primitives, migrated under the `collector` namespace.
...
/// Build the collector router over the shared `conn`, mirroring `tunnel-rust`'s
/// `setup_tunnel` and `apps-rust`'s `setup_apps`. The host opens one database
/// and passes it in; constructing the store applies the collector-namespaced
/// migrations (including the demo-remote seed).
///
...
pub fn setup_collector(conn: persistence_rust::Connection) -> anyhow::Result<Router> {
    let store = RemotesStore::new(conn).context("failed to open remotes store")?;
