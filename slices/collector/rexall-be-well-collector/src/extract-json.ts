 ...
 * (`<` → `&lt;`, `&` → `&amp;`, etc.).
 *
 * The Rexall SPA's medication/profile payloads arrive as XHR intercepts
 ...
 * `extract-json.ts` — slice layering forbids one `*-client-collector`
 * importing another.
 *
 * Strategy:
 ...
 *
 * Not in scope:
 *   - Multi-byte / surrogate-pair entities (`&#xFFFD;` etc.) — FHIR
 ...
 *     `application/json`; if the tunnel ever serves `text/html`
