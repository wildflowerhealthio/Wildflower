...
/**
 * The Rexall collector's persist sink: the descriptor's `persistResources`.
 ...
 *
 * This is a verbatim copy of `fhir-r4-client-collector/src/persist.ts`: both
 * collectors target the same on-device FHIR R4 store through the same
    ...
    (resource) => {
      ...
      return upsertResource(resource).pipe(
        ...
        Effect.tapError((cause) =>
          Effect.logError(`rexall persist: upsert failed for ${failed.label}/${failed.id}`, cause)
        ),
