...
/**
 * Loader helper for authed collector routes. The `_auth` layout now
 * gates on a `beforeLoad` that `await`s the bearer token, so by the
 * time this loader runs the token is guaranteed present — embedded
 * waited the bridge handshake, web had it synchronously. No more
 * first-paint skip; this is a plain `ensureQueryData`.
 *
