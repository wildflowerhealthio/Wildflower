...
//! rendered here is the opaque 500 for a [`Backend`](TunnelError::Backend)
//! failure — logged via the shared [`InternalError`], answered as an empty body
//! a client doesn't decode. `Backend` is therefore deliberately **not** modeled
//! in the routes' `#[utoipa::path]` responses (mirroring collector's
//! `RemoteError::Backend`).
//!
...
/// [`Backend`](TunnelError::Backend) variant is logged (via the shared
...
impl IntoResponse for TunnelError {
    fn into_response(self) -> Response {
        match self {
            TunnelError::Backend { context, source } => {
