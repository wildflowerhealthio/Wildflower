...
- **Authority proofs** — every privileged write (approve a request, issue a code, record a grant, mint a token, issue a refresh token) happens in a `domain/capabilities/writers/` method that takes a proof type from `domain/authority/` (`AuthenticatedClient`, `DelegatedScopes`, `StandingGrantCoverage`, the redemptions, `HostBootstrap`). Audit by reading each proof's single constructor and each writer; source guards pin the privileged store methods to the writers, the host-bootstrap proof to `seeding.rs`, and every handler and middleware file to the capability extractors. See the How-To's "Authority proofs" section
