...
- [Scope-Gated Endpoints How-To](../../docs/Authorization/Scope-Gated%20Endpoints%20How-To.md) — the `/access` surface's scope-gated `Scoped<F>`/facade pattern (now shared from `shared-structures-rust`); `http/scoped/facades/` is the worked fixed-scope example
