...
// Bootstrap: pull the bearer token from one of two sources, stash to
// localStorage so the HttpApi client can read it on every request:
//   1. `window.__GATEKEEPER_TOKEN__` — set by the RN host before the
//      bundle runs (see `gatekeeper-expo/GatekeeperWebView`).
//   2. `?token=` — set when the SPA is loaded standalone in a browser.
//      Stripped from the address bar so it doesn't persist in history
//      or Referer headers. See gatekeeper-core README's "Bootstrap URL".
