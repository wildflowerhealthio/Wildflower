 ...
 * half of the flow, run only after a user confirms an {@link ImportPreview}.
 * Exposed as `ImportPreview.persist` through the `import-preview.ts` barrel.
 */
 ...
 * it came from.
 *
 * @param preview - The preview to persist; a `NoImporterClaims` writes nothing
 ...
 * carried — the same single-valued back-pointer the live collection path writes, here
 ...
 *
 * A `NoImporterClaims` preview has nothing to write, so this is an immediate
