...
/**
 * The result of previewing a HAR import: either no registered importer
 * understood the archive, or one did and here is what it would write.
 *
 * @remarks
 * A tagged union rather than an error channel, because "no importer claims this
 * traffic" is an ordinary, first-class outcome the caller renders — a browser's
 * HAR export of a site we have no importer for is not a failure, it is a
 * `NoImporterClaims`. The only thing that reaches an Effect error channel from
 ...
 *
 * @remarks
 * The archive was never captured for our purpose, so a response that matched an
 * importer's URL pattern but did not decode as the resource that pattern
...
/**
 * No registered importer claimed the archive's traffic.
 *
 * @remarks
 * A first-class outcome, not an error: an archive of a site we have no importer
 ...
 */
interface NoImporterClaims {
  readonly _tag: 'NoImporterClaims'
  readonly totalEntries: number
}

/**
 * A registered importer claimed the archive, and this is what running its
 ...
 * it arrived on by the importer's entities — the ids here are the ids a
 ...
 * and the importer's entities key each resource under the root of *its own* URL,
...
type ImportPreview = NoImporterClaims | Preview

export { persist } from './persist-preview.ts'
export type { ImportParseFailure, ImportPreview, NoImporterClaims, Preview }
