...
/**
 * The result of previewing a HAR import: either no registered collector
 * understood the archive, or one did and here is what it would write.
 *
 * @remarks
 * A tagged union rather than an error channel, because "no collector claims this
 * traffic" is an ordinary, first-class outcome the caller renders — a browser's
 * HAR export of a site we have no collector for is not a failure, it is a
 * `NoCollectorClaims`. The only thing that reaches an Effect error channel from
 * {@link runHarImport} is a malformed archive (a `ParseError` from the HAR
 ...
 * {@link persistPreview} write. This type is the whole contract between the two
 * halves of that flow.
 */
 ...
 * collector's URL pattern but did not decode as the resource that pattern
 * promises is a reportable outcome, not a run-ending error — the same stance
 * `Replay.replayEntities` takes. `url` locates it for a reviewer; `error` is the
...
/**
 * No registered collector claimed the archive's traffic.
 *
 * @remarks
 * A first-class outcome, not an error: an archive of a site we have no collector
 ...
 */
interface NoCollectorClaims {
  readonly _tag: 'NoCollectorClaims'
  readonly totalEntries: number
}

/**
 * A registered collector claimed the archive, and this is what replaying its
 * offline entities produced — grouped for display, with every non-resource
 * outcome counted.
 *
 * @remarks
 ...
 * it arrived on by the collector's offline entities — the ids here are the ids a
 * subsequent {@link persistPreview} writes. The three counts and
 ...
 *   `Replay.ReplayResponse.bodyAbsent`).
 *
 ...
 * and the offline entities key each resource under the root of *its own* URL, so
 * a two-server archive keeps both roots here and keys each server's resources
...
/** The two outcomes of {@link runHarImport}. */
type ImportPreview = NoCollectorClaims | Preview

export type { ImportParseFailure, ImportPreview, NoCollectorClaims, Preview }
