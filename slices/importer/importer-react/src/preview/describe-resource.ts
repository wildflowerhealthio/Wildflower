 ...
 * (`PreviewedResource<unknown>`) even though the HAR binding pins `TParsed =
 * FhirResource` at runtime. The read is a per-type Effect Schema decode against
 * the summary fields we care about (never the whole resource), so a partial or
 * mis-typed field folds cleanly to `''` instead of throwing, and the summary
 * falls back to `resourceType/id` when nothing survives.
 *
