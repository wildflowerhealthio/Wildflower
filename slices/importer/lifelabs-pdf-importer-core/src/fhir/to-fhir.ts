...
import type { LifeLabsReport, ReportPatient, ReportRow } from '../dialect/report.ts'
...
/**
 * The FHIR R4 synthesis: LifeLabs reports in, `Patient` / `Practitioner` /
 * `DiagnosticReport` / `Observation` resources out, each keyed by an id the
 * report itself determines so a re-import lands on the same records.
 *
 * @remarks
 * The resources are built as plain wire objects and decoded through `fhir-r4`'s
 * schemas, so an ill-formed synthesis is a `ParseError` here rather than a
 * write failure later. Every reference is spelled relatively (`Patient/<id>`)
 * against the *source* ids this module mints; the response kind's adoption
 * re-keys both sides under the LifeLabs source system, which is what keeps a
 * `DiagnosticReport.result` pointing at the `Observation` it names.
 *
 * What is deliberately left out (LOINC, `Organization`, `ServiceRequest`) and
 * why is in this package's AGENTS.md, under Guardrails.
 *
 ...
 */
const patientOriginalId = (patient: ReportPatient): string => {
 ...
 */
const reportOriginalId = (report: LifeLabsReport): string =>
...
const patientWire = (report: LifeLabsReport, generalPractitionerId: string | undefined): Wire => {
...
const performerWire = (report: LifeLabsReport, licence: string): Wire[] => {
...
const referenceRangeWire = (row: ReportRow): Wire[] => {
...
const valueWire = (row: ReportRow): Wire => {
...
const reportStatus = (report: LifeLabsReport): string =>
 ...
 *
 * @remarks
 * One `Observation` per grid row, whatever the row is — a numeric assay, a
 * `NEGATIVE`, a `Collection Date` — because every row is a result line the
 * report prints, and interpreting some as metadata would drop them. Section
 * and group names ride on the observation's `category.text`; the row's
 * printed comments become its one `note`.
 */
