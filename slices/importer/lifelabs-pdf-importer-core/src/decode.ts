...
import { PositionedTextFromJson } from 'pdf-anonymizer-core'
...
const decodeDocument = Schema.decodeUnknown(PositionedTextFromJson)
 ...
 *
 * @remarks
 * The body is the file's own text, not a re-serialization of the decoded
 * document: the decode here is a gate (is this the format at all?), and the
 * response kind decodes the same bytes again when it parses — the same
 * gate-then-parse split the HAR binding's picker makes.
 */
