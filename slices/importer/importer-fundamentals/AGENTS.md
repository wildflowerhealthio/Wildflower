...
The vocabulary of importing: everything needed to reason about "an importer" —
a thing that recognizes archived traffic and decodes it into resources —
without any reference to the live collector machinery. The collector slice
builds on this package, never the reverse.

## Namespaces

One namespace per module, in the `effect` style: the file is the noun, the
principal type shares the namespace's name (`Importer.Importer`), and
functions read in the namespace's context (`Extraction.run`, not
`runExtraction`). All six are exported from the **flat root entry**:
`import { EntityDefinition, Extraction, Importer } from 'importer-fundamentals'`.

- **`EntityDefinition`** (`src/entity-definition.ts`) — the recipe for
  recognizing and decoding one response shape: `name` / `isFoundAt` / `parse`.
  `parse` takes an `ImportableResponse` and returns
...
- **`ImportableResponse`** (`src/importable-response.ts`) — what a `parse`
  sees: `id` / `url` / `status` / `statusText` / `headers` (`Headers`) /
  `startedAt` / `bytes()` / `text()`. An interface, not a class: `make` builds
  one over bytes already in hand (the extraction path), and
...
- **`Extraction`** (`src/extraction.ts`) — run archived responses through an
  importer's entities. `Extraction.run(entities, inputs)` folds each
  `Extraction.Input` (the `ImportableResponse` data fields + `body` +
  `bodyAbsent`) through first-`isFoundAt`-match-wins routing into an
  `Extraction.Extraction`: the four-way accounting of `batches` / `unmatched`
  / `parseFailures` / `bodyAbsent`, every input in exactly one bucket, in
  input order. Infallible — failures are data, not errors. It never persists.
- **`Recognizer`** (`src/recognizer.ts`) — how an importer claims a response
  set with no user configuration (`name` / `specificity` / `claims`);
  `Recognizer.resolve` picks the most specific claimant. Specificity
  convention: portal-specific > protocol-generic > catch-all.
- **`Importer`** (`src/importer.ts`) — an importer as one first-class value:
  `Importer.Importer<TResources>` extends `Recognizer.Recognizer` with `tag`,
  `entitiesFor(archive)` (a factory over `Importer.ArchiveContext`, so an
  importer whose identity is per-archive can derive it), and `rootOf(url)`.
  Concrete importer projects assemble one (`fhir-r4-importer`'s
  `fhirR4Importer`) and `importer-core`'s closed `importers` list registers
  them.

`importer-fundamentals/test-helpers` is the one sub-entry:
`makeImportableResponse`, `makeExtractionInput`, the `SimpleEntity` /
...
- **This package imports from no slice.** Its dependencies are exactly
  ...
  `*-client-collector`, an importer project, or `importer-core`.
- The live-vs-offline parity pin — that `Extraction.run` and the collector's
...
- **`bytes()` returns a fresh copy each call** (both `ImportableResponse.make`
  and the live implementation) so a caller cannot mutate the body out from
  under a later reader.

## References

- [slices/importer/AGENTS.md](../AGENTS.md) — the slice this package anchors.
- [slices/collector/AGENTS.md](../../collector/AGENTS.md) —
  `collector-fundamentals`, the live machinery built on this vocabulary.
