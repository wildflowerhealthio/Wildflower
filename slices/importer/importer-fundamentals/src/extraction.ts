 ...
 * {@link ImportableResponse.ImportableResponse} exposes, with the body
 * already in hand.
 *
 * @remarks
 ...
 * instead.
 */
interface Input extends ImportableResponse.Source {
...
/**
 * Run a static set of archived responses through an importer's entities.
 *
 * @typeParam TResources - The resource type the entities decode to
 * @param entityDefinitions - The importer's entities, in list order; the
 ...
 * disjoint), same {@link ImportableResponse.ImportableResponse} handed to
...
const run = <TResources>(
  ...
  Effect.gen(function* () {
    ...
    for (const source of responses) {
      ...
      const parsed = yield* Effect.either(entity.parse(ImportableResponse.make(source)))
