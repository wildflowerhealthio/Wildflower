...
/**
 * The plain data {@link make} builds an {@link ImportableResponse} from: the
...
/**
 * Build an {@link ImportableResponse} over bytes already in hand.
 *
 * @param source - The response fields and the complete body
 * @returns An `ImportableResponse` whose `bytes()` returns a fresh copy of
 *   `source.body` and whose `text()` is its UTF-8 decode
 */
const make = (source: Source): ImportableResponse => ({
  id: source.id,
  url: source.url,
...
export { make }
export type { Headers, ImportableResponse, Source }
