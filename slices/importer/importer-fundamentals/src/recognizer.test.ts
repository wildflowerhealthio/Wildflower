...
/** A recognizer carrying a caller-owned payload, as a real `Importer` would. */
...
): Registered => ({ name, specificity, claims, importerTag: `${name}-tag` })
...
describe('Recognizer.resolve', () => {
  ...
  it("carries the caller's payload through untouched", () => {
    ...
    expect(Option.map(winner, (r) => r.importerTag)).toEqual(Option.some('portal-tag'))
  })
