import { DateTime, Effect, Encoding, Option, Schema, TestContext } from 'effect'
...
const exampleFormat: SourceFile.FormatValue = {
...
/** The same schemas under a second format — what "another format's document" means below. */
const otherFormat: SourceFile.FormatValue = {
  ...exampleFormat,
...
const runAs = <A, E>(
  ...
  effect: Effect.Effect<A, E, SourceFile.Format>
): Promise<A> =>
  Effect.runPromise(
    effect.pipe(
...
const run = <A, E>(effect: Effect.Effect<A, E, SourceFile.Format>): Promise<A> =>
  runAs(exampleFormat, effect)

const mintSchema = Schema.decode(SourceFile.FromNamedBytes)
const encode = Schema.encode(SourceFile.FromDocumentReference)
const readBack = Schema.decode(SourceFile.FromDocumentReference)
const readContents = Schema.decode(SourceFile.NamedBytesFromDocumentReference)

const mint = (fileName: string, bytes: Uint8Array): Promise<SourceFile.Type> =>
  run(mintSchema({ fileName, bytes }))

const nameArbitrary = fc.stringMatching(/^[a-z0-9]{1,8}\.bin$/u)

describe('FromNamedBytes — the deterministic mint', () => {
  ...
  it('namespaces the id by the coding system, so two formats never collide', async () => {
    ...
    const here = await runAs(exampleFormat, mintSchema(picked))
    const there = await runAs(otherFormat, mintSchema(picked))
    expect(here.id).not.toBe(there.id)
  })

  it('keeps the name and the bytes verbatim, and links to nothing yet', async () => {
    ...
    expect(Option.isNone(sourceFile.subject)).toBe(true)
    expect(sourceFile.related).toEqual([])
  })

  it('encodes back to the name and the bytes it was minted from', async () => {
    const bytes = new Uint8Array([1, 2, 3])
    const sourceFile = await mint('binary.bin', bytes)
    const picked = await run(Schema.encode(SourceFile.FromNamedBytes)(sourceFile))
    expect(picked).toEqual({ fileName: 'binary.bin', bytes })
  })
})

// ---------------------------------------------------------------------------
// FromDocumentReference — the codec proper
// ---------------------------------------------------------------------------

const UPLOAD_FLOOR = Date.UTC(2026, 0, 1)

const subjectArbitrary: fc.Arbitrary<SourceFile.Subject> = fc
  .uuid()
  .map((id) => ({ reference: `Patient/${id}` }))
...
const sourceFileArbitrary: fc.Arbitrary<SourceFile.Type> = fc.record({
  ...
  subject: fc.option(subjectArbitrary, { nil: undefined }).map(Option.fromNullable),
  related: fc.array(
    fc.uuid().map((id): SourceFile.Subject => ({ reference: `ImagingStudy/${id}` })),
    { maxLength: 3 }
  ),
})
...
const roundTrip = (sourceFile: SourceFile.Type): Promise<SourceFile.Type> =>
  run(encode(sourceFile).pipe(Effect.flatMap(readBack)))
...
describe('the codec as a schema', () => {
  test('property: an upload round-trips — id, filename, instant, bytes and links recovered exactly', async () => {
    await fc.assert(
  ...
  test('property: hash and size describe the attachment bytes, stored verbatim', async () => {
    await fc.assert(
      fc.asyncProperty(sourceFileArbitrary, async (sourceFile) => {
        const attachment = (await run(encode(sourceFile))).content[0]?.attachment
  ...
  test('property: subject is absent unless the value names one, keeping source files out of Patient/$everything', async () => {
    await fc.assert(
      fc.asyncProperty(sourceFileArbitrary, async (sourceFile) => {
        const resource = await run(encode({ ...sourceFile, subject: Option.none() }))
  ...
  test('property: a subject on the value files the resource under exactly that reference', async () => {
    await fc.assert(
      fc.asyncProperty(sourceFileArbitrary, fc.uuid(), async (sourceFile, patientId) => {
        const resource = await run(
          encode({ ...sourceFile, subject: Option.some({ reference: `Patient/${patientId}` }) })
        )
  ...
  test('property: every related resource on the value is named as context.related', async () => {
    await fc.assert(
      fc.asyncProperty(sourceFileArbitrary, fc.uuid(), async (sourceFile, studyId) => {
        const resource = await run(
          encode({ ...sourceFile, related: [{ reference: `ImagingStudy/${studyId}` }] })
        )
  ...
  test('property: context is absent unless a related resource is named', async () => {
    await fc.assert(
      fc.asyncProperty(sourceFileArbitrary, async (sourceFile) => {
        expect((await run(encode({ ...sourceFile, related: [] }))).context).toBeNull()
      }),
      { numRuns: numRunsFor({ base: 50 }) }
    )
  })
  ...
  it("another format's document is not this one's source file", async () => {
    ...
    expect(SourceFile.isSourceFile(exampleFormat)(resource)).toBe(false)
    expect(SourceFile.isSourceFile(otherFormat)(resource)).toBe(true)
  })
})

describe('the constants a reader of the server list projects', () => {
  it('states the category token in system|code form', () => {
    expect(SourceFile.categoryToken(exampleFormat)).toBe(`${SYSTEM}|example-source-file`)
  })

  it('recognizes an archive of its own format', async () => {
    const resource = await run(encode(example()))
    expect(SourceFile.isSourceFile(exampleFormat)(resource)).toBe(true)
  })
})

describe('NamedBytesFromDocumentReference — what a preview reads', () => {
  test('property: the stored name and bytes come back in one decode', async () => {
    await fc.assert(
      fc.asyncProperty(sourceFileArbitrary, async (sourceFile) => {
        const contents = await run(encode(sourceFile).pipe(Effect.flatMap(readContents)))
        expect(contents.fileName).toBe(sourceFile.fileName)
        expect(contents.bytes).toEqual(sourceFile.bytes)
      }),
      { numRuns: numRunsFor({ base: 50 }) }
    )
  })
