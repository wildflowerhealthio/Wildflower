/**
 * The codec that turns a source file into the FHIR `DocumentReference` it is
 * stored as, and back.
 *
 * @remarks
 * Written once here rather than per format, parameterised by the
 * {@link FormatContext} a format's coding constants supply. `FileImporter.make`
 * is the only place that binds that context; see `file-importer.ts`.
 *
 * The vocabulary the codec reads and writes — `SourceFile.Type`,
 * `SourceFile.Coding`, `SourceFile.Reference` — is `source-file.ts`, which
 * knows nothing of this module. That direction is deliberate: naming a source
 * file is cheap and context-free, encoding one is neither.
 *
...
const decodeParts: (
  parts: PartialEncoded
) => Effect.Effect<SourceFile.Type, ParseResult.ParseIssue> =
  ParseResult.decodeUnknown(SourceFileSchema)

const decodeResource = ParseResult.decodeUnknown(DocumentReference.Schema)

/**
 * The per-format constants the codec is parameterised by: which coding a
...
const hasCoding = (
  concept: FhirR4.CodeableConcept | undefined,
  coding: SourceFile.Coding
): boolean =>
...
const decode = (
  wire: FhirR4.DocumentReference,
  ast: SchemaAST.AST
): Effect.Effect<SourceFile.Type, ParseResult.ParseIssue, FormatContext> =>
...
): Effect.Effect<SourceFile.Type, ParseResult.ParseError, FormatContext> =>
  Effect.gen(function* () {
 ...
 * `SourceFile.Type`, this yields the stored resource. The batch decode does the
 * two steps separately, because it can only name the subject once the file's
 * decode has run.
 *
 * @param picked - The picked file's name and bytes
 * @param subject - The subject to file it under, when the format names one
