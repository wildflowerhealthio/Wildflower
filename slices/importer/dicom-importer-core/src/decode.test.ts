...
describe('decodeDicom', () => {
  it('yields one section with Patient, ServiceRequest, ImagingStudy', async () => {
    const result = await Effect.runPromise(decodeDicom(sampleDicomBytes(), defaultDicomSettings))
  ...
  it('notes no ServiceRequest when AccessionNumber is absent', async () => {
    ...
    const result = await Effect.runPromise(decodeDicom(bytes, defaultDicomSettings))
  ...
  it('fails with ParseError on truncated bytes', async () => {
    const result = Effect.runSync(
      Effect.either(decodeDicom(new Uint8Array([0x00, 0x01, 0x02]), defaultDicomSettings))
    )
    expect(result._tag).toBe('Left')
  })

  it('resource keys are stable across the empty settings', async () => {
    const result1 = await Effect.runPromise(decodeDicom(sampleDicomBytes(), defaultDicomSettings))
    const result2 = await Effect.runPromise(decodeDicom(sampleDicomBytes(), defaultDicomSettings))
