...
- **`har-importer-core`** (the HAR binding) — the concrete
  ...
  (`fhirPool` = `fhir-r4-source`'s pre-adopted `fhirR4SourceEntities`), and the
  FHIR persist sink (`persistFhir` = `withMetaSource` + `persistResources`). Bound
  to `TResource = FhirResource`, empty `HarSettings`. No DOM, no `fs`, no React.
...
- **`http-extraction-fundamentals`** (in `slices/http-extraction`) owns the
  ...
  type.
- **`fhir-r4-source`** (same slice) owns the FHIR R4 response kinds and their
  pre-adopted `fhirR4SourceEntities` but knows nothing about HAR or about a
  registry of formats.
