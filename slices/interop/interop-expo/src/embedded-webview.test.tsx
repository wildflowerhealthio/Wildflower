...
jest.mock('react-native-webview', () => {
  ...
  // `webviewRef.current` stays null inside EmbeddedWebView; its own
...
import { EmbeddedWebView, type EmbeddedWebViewHandle } from './embedded-webview.tsx'
...
describe('EmbeddedWebView (transport surface)', () => {
  ...
  it('does not set a headerLeft when canGoBack is omitted', () => {
    render(<EmbeddedWebView source={{ html: '' }} onMessage={jest.fn()} />)
  ...
  it('sets a headerLeft when canGoBack flips to true with onBackPress provided', () => {
    const { rerender } = render(
      <EmbeddedWebView source={{ html: '' }} onMessage={jest.fn()} canGoBack={false} />
    )
    rerender(
      <EmbeddedWebView
        source={{ html: '' }}
  ...
  it('exposes postMessage through the imperative ref handle', () => {
    const ref = React.createRef<EmbeddedWebViewHandle>()
    render(<EmbeddedWebView ref={ref} source={{ html: '' }} onMessage={jest.fn()} />)
  ...
  it('dismisses the loading overlay on onLoadEnd', () => {
    const { queryByTestId } = render(
      <EmbeddedWebView source={{ html: '' }} onMessage={jest.fn()} />
    )
