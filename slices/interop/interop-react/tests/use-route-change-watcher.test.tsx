...
import { type RouteChangedMessage, useRouteChangeWatcher } from '../src/use-route-change-watcher.ts'
...
describe('useRouteChangeWatcher', () => {
  test('fires the callback once per navigation, with canGoBack tracking history depth', () => {
    const calls: RouteChangedMessage[] = []
    const send = (message: RouteChangedMessage): void => {
      calls.push(message)
    }
