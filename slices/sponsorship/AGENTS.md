...
- `sponsorship-core` — the pure layer. Province model, the normalized
...
- Keep `sponsorship-core` FHIR-agnostic. The FHIR `MedicationRequest` mapping
  lives in `sponsorship-react` (it already depends on `fhir-r4`).
