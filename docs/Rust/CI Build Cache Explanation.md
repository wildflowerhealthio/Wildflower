...
Every one of them sets `cache-on-failure: true`, so a red run still saves the
...
`rust-toolchain.toml`) and saves under `workspace-linux-dev`.
...
of `clippy-all` / `test-all`, which shares their
`--workspace --all-targets --all-features` selection: feature unification and
target selection are part of an artifact's fingerprint, so a warm-up that
differed on either would cache artifacts CI then rebuilds.
