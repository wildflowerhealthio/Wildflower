...
The importer slice does the same with a schema rather than callbacks: `importer-fundamentals`' archive codec carries a `PickedFile.Format` requirement, and `DecodeFunction.make` provides it from the `sourceFileFormat` the binding handed it, so the batch `decode` it returns requires nothing. The tag's only other provider is the shell's read of a stored archive, from the same registry entry's constants — which is what keeps a format's coding spelled once. A constructor that closed over the constants instead would let two copies disagree.
