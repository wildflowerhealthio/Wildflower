...
| `@react-navigation/native` | `^7.2.4`  | `expo-router@55.0.14` has a direct dep on `@react-navigation/native@^7.1.33`, but its sibling deps `@react-navigation/bottom-tabs@7.15.5` → `@react-navigation/elements@2.9.14` and `@react-navigation/native-stack@7.14.10` declare `@react-navigation/native@^7.2.2` as a peer. Without the override, pnpm installs both 7.1.33 and 7.2.4 side-by-side and the inner navigators are wired to the 7.1.33 instance, breaking the peer contract. |
