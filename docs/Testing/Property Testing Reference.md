...
- **Decompose by column.** Round-trip one field at a time via `Schema.pick(name)` instead of the whole schema. Drove `emr-core` from 745s / 14 timeouts to ~78s / 0. Drive it from a `test.each(columnCases)` where `columnCases` is `{ name, numRuns? }[]` so per-column overrides stay readable. Two viable variants: pick-only round-trip (fastest — used in `emr-core`) and shell-spread + whole-schema round-trip (used in `fhir-r4` to keep wire-format adapter coverage end-to-end).
