...
**Learning**: Encoding through a refinement first decodes the in-memory value against the schema's Type side, then encodes the result. A `Schema.declare` is its own Type side, so a declaration whose decode maps any input to `null` (fhir-r4's unregistered-datatype stub) silently turned a non-null in-memory slot into `null` once the struct was filtered, and the loud encode failure vanished. Composing the declaration onto `Schema.Null` gives it a strict Type side. Separately, `Schema.omit` / `Schema.pick` rebuild a plain struct and discard every refinement on the way — `withMandatoryId` has to re-apply them. A `jsonSchema` annotation with a `type` key on a refinement overrides the generated JSON Schema, so an OpenAPI snapshot is unchanged when the annotation moves onto the filter.
**Suggested destination**: Effect Patterns Reference
...
## Documenting a gated route in OpenAPI means splitting its `OpenApiRouter`

**Discovered during**: ruthmarks/owner-manage-clients (#162, `/access/clients`)
**Learning**: gatekeeper-rust's `documented_router()` fed both the served router and the spec, so nesting a new `/access/*` `OpenApiRouter` there would have served it _outside_ the `require_valid_session` layer that wraps the rest of `/access`. The fix is to build the gated routes once as an `OpenApiRouter` and split it twice: the axum half merges into the session-gated `access` router, the spec half nests under `/access` in a spec-only `documented_router()` (the served pre-auth half is `documented_public_router()`). Pair it with `InsufficientScopeResponses::for_paths` (needs `scope-capabilities-rust`'s `openapi` feature) so the spec carries the `403` that the TS group declares, and feed it the spec's own `/access/` path keys rather than a hand-kept list that a new route can miss. Two drift details: an `Option<…>` field serde always writes (as `null`) needs `#[schema(required = true)]` to match an Effect `Schema.NullOr`, and `DateTime<Utc>` needs `value_type = String` since the workspace's utoipa has no `chrono` feature.
**Suggested destination**: OpenAPI Spec Drift How-To, and the Scope-Gated Endpoints How-To

## A section-local suspense query needs `QueryErrorResetBoundary`, and commits on the next `act` in jsdom
