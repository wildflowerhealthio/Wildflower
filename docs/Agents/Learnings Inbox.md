...
## Toggling a build-time `EXPO_PUBLIC_*` flag per-test: read it in the render body, don't `isolateModules`-reimport

**Discovered during**: ruthmarks/webview-dev-spa-url — testing `AppShellWebView`'s `EXPO_PUBLIC_DEV_SPA_URL` dev/embedded mode switch under jest-expo
**Learning**: A module-level `const flag = __DEV__ ? process.env.EXPO_PUBLIC_X : undefined` is evaluated once at import, so a single test file's static import bakes one value — you can't exercise both the set and unset branches. Two dead ends under jest-expo (Jest 29): (1) dynamic `await import('./mod')` inside `jest.isolateModulesAsync` throws `A dynamic import callback was invoked without --experimental-vm-modules` — jest-expo's babel leaves `import()` as a native dynamic import, not a `require`. (2) `jest.isolateModules(() => require('./mod'))` (or `requireActual`) DOES re-evaluate the module, but it pulls a _second copy of React_ into the isolated registry; rendering that component with the outer `@testing-library/react-native` renderer fails with `Cannot read properties of null (reading 'useState')` (mismatched dispatcher). Also `require()`'s `any` return trips the repo's `no-unsafe-assignment` lint. The clean fix is a one-line source move: read `const devSpaUrl = __DEV__ ? process.env.EXPO_PUBLIC_DEV_SPA_URL : undefined` **inside the component body** instead of at module scope (add it to any `useMemo` dep array that uses it). Metro still inlines both `__DEV__` and the `process.env.EXPO_PUBLIC_*` token, so production behaviour is identical (a build-time constant); but jest-expo leaves `process.env.EXPO_PUBLIC_*` a genuine _runtime_ read (verified: a function reading it picks up a value `process.env.X = …` set after transform — same reason telemetry-react-native's env tests work), so the test just sets/restores `process.env.EXPO_PUBLIC_X` around `mountInPipe(<Component/>)` with no re-import and no React-instance hazard. Repo example: `apps/wildflower-expo/src/app-shell/app-shell-webview.test.tsx`'s `dev-SPA mode` describe.
**Suggested destination**: docs/Testing/Unit Testing How-To.md
