...
(1) **`stop()` resolves the task wrapper immediately** — it calls the internal `_stopTask()` resolver and tears down the native service *without awaiting your task*. So graceful teardown (e.g. `Fiber.interrupt` on the daemon launch, awaited so the HTTP listener/tunnel finalizers run) has to live INSIDE the task body, gated on a stop cue — not after `await stop()`.
