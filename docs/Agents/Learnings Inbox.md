...
**Learning**: `Effect.withSpan` only produces an OTel span when the *running fiber* has the tracer set in its FiberRef (`@effect/opentelemetry`'s `OtelEffectTracer.layerGlobal` sets it). Code that runs inside a `forkScoped`/`forkDaemon` fiber — e.g. the effect-messaging transport's inbound-dispatch fiber and the collector step-timers — inherits FiberRefs *as a snapshot taken at fork time*, so the span calls in those handlers are silent no-ops unless the tracer was already in context when the fork happened. Fix: provide the telemetry layer to the transport-build effect at boot (`apps/wildflower-react/src/bridges/build-transport.ts` → `Effect.provide(webTelemetryLayerFromEnv())`); the forked fibers then snapshot it. You do NOT need the effect to *require* the tracer in its `R` — providing the layer sets the FiberRef as a side effect, and the snapshot survives even after the provide-scope closes (FiberRefs are per-fiber copies). Second gotcha, the one that bit the unit test: `webTelemetryLayerFromEnv()` is NOT pure even on the disabled path — `makeClientTelemetryLayer` calls `initClientTelemetry` eagerly at layer-*construction* time, which installs a global OTel `StackContextManager` and inits Sentry *before* the `isTelemetryEnabled` check returns `Layer.empty`. So a focused unit test that drives `buildTransport` with everything else mocked must also `vi.mock('telemetry-web', () => ({ ...actual, webTelemetryLayerFromEnv: () => Layer.empty }))` — otherwise those global side effects perturb the test's microtask timing and the shared OTel/Sentry globals leak across tests (observed as a `signalReady` timeout + a doubled console-interceptor call). `initClientTelemetry` is idempotent (`if (registered !== undefined) return registered`), so calling it from `buildTransport` in addition to the app-boot `initWebTelemetryFromEnv()` is safe in production.
