...
**Learning**: The importer carried `PickedFile.Source` — `local` or `server`, the latter holding the `DocumentReference` reference a re-picked file was fetched from — so every step below the picker had two arms: mint an archive or don't, stamp a fresh reference or the carried one. The tag existed only to avoid writing the same archive twice, and it was not needed for that: the archive's id is already a hash of the bytes, the name and the coding system, so re-picking a stored file mints exactly the archive it came from. Archiving unconditionally collapsed the branch, and the server-diff step the shell already runs classifies the re-minted row `unchanged`, which the initial selection pre-excludes — the "don't upload it twice" behaviour, arrived at by the mechanism that was there anyway. Generalize to: when a value carries a tag whose only job is to skip an idempotent operation, check whether the operation is idempotent enough to just run.
**Suggested destination**: Strategies
