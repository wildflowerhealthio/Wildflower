...
**Learning**: `@fast-check/vitest`'s `it.prop(...)` registers its tests against the `vitest` package's runner instance, which is a different module instance than `vite-plus/test`'s — so collection dies with `Error: Vitest failed to find the current suite. This is a bug in Vitest.` (it isn't; it's the dual-runner mismatch, same family as the known `@effect/vitest` breakage). The repo convention that works: `import * as fc from 'fast-check'`, write properties inside a normal `it(...)` as `await fc.assert(fc.asyncProperty(...), { numRuns: numRunsFor({ base: 100 }) })` with `numRunsFor` from `kitchen-sink/test` (which is also what lets `test:changed` scale runs down). See `effect-messaging-core/src/logging.test.ts` for the canonical shape. The `/javascript-testing-expert` skill's appendix recommends `@fast-check/vitest` — don't follow it on this repo.
**Suggested destination**: docs/Testing/Property Testing Reference.md (and fix the skill appendix)
...
## `process.cwd()` in a Vitest projects-mode worker is the monorepo ROOT, not the package — `numRunsFor`'s risk scaling was a silent no-op

**Discovered during**: ruthmarks/fix-num-runs-for — verifying `test:changed`'s `FC_RISK_MAP` scaling
**Learning**: `vp run test:changed` sets `FC_RISK_MAP="$(node scripts/risk-map.ts)"` (a `{ <pkgName>: <multiplier> }` map) and runs `vp test` from the repo root. The env var DOES propagate into Vitest worker threads. But the old `global/kitchen-sink/src/test/num-runs-for.ts` resolved the current package from `<process.cwd()>/package.json` — and under Vitest projects mode the worker's `process.cwd()` is the directory the runner was launched from (the repo ROOT), NOT the per-package dir. So it read `wildflower-monorepo`, which is never a key in the risk map (and `risk-map.ts` never emits the `'*'` wildcard the helper also checks), so the multiplier always defaulted to `1.0`: scaling never reached fast-check. It only "worked" when `vp test` was run from inside a single package dir (which `test:changed` never does). Empirical proof: from root with `FC_RISK_MAP={"kitchen-sink":0.2}`, `cwd`-based `numRunsFor(100)` returned 100; the same from the package dir returned 20. Fix: resolve the package from the _calling test file_ — prefer Vitest's per-running-test-file `expect.getState().testPath` (survives projects mode and stays correct even if a future shared in-repo helper wraps the call), and fall back to a call-stack walk (`new Error().stack`, skip this module's own frames matched by `fileURLToPath(import.meta.url)` and `node_modules`, then walk up to the nearest named `package.json`) for non-Vitest callers — then resolve the multiplier per call, not once at module load (one worker can run test files from more than one package). Another usable per-test-file signal: `globalThis.__vitest_worker__.filepath`.
**Suggested destination**: docs/Testing/Property Testing Reference.md (or Testing Reference's `test:changed` note)
...
## `react-native-background-actions`: `stop()` doesn't await the task, `expiration` doesn't flip `isRunning()`, and Android 14 needs a `foregroundServiceType`

**Discovered during**: ruthmarks/background-actions-server — moving the on-device HTTP-server + tunnel launch into a background foreground service
**Learning**: Three non-obvious facts from the lib's source (`node_modules/react-native-background-actions/src/index.js`), all load-bearing when wrapping an Effect daemon launch in `BackgroundService.start(task, options)`:
(1) **`stop()` resolves the task wrapper immediately** — it calls the internal `_stopTask()` resolver and tears down the native service _without awaiting your task_. So graceful teardown (e.g. `Fiber.interrupt` on the daemon launch, awaited so the HTTP listener/tunnel finalizers run) has to live INSIDE the task body, gated on a stop cue — not after `await stop()`.
(2) **The stop cue is poll-only for the `stop()`/Android/unmount path** (`BackgroundService.isRunning()` flips to `false`), but **iOS `expiration` does NOT flip `isRunning()`** — `isRunning()` stays `true` until `stop()` is called. So the task must subscribe to the `'expiration'` event (`BackgroundService.on('expiration', …)`) separately; polling alone never tears down on iOS time-expiry. Make the teardown idempotent since both cues can fire.
(3) **`start()` runs the task in-process on iOS** (`finalTask()` is invoked synchronously after native start) but registers a HeadlessJS task on Android. A double `start()` (React StrictMode mount→unmount→mount, or a fast `requestedRunning` toggle) thus binds the server twice on iOS → port conflict. Serialise start/stop through a single promise chain that reads desired-state at execution time so a burst collapses to one start.
Native config (no official Expo plugin): add `FOREGROUND_SERVICE` / `FOREGROUND_SERVICE_DATA_SYNC` / `WAKE_LOCK` to `app.json` `android.permissions`, pass `foregroundServiceType: ['dataSync']` in the JS options, AND stamp `android:foregroundServiceType="dataSync"` onto the lib's `com.asterinet.react.bgactions.RNBackgroundActionsTask` service via a local `withAndroidManifest` config plugin (the lib's own manifest declares the service without a type, which Android 14+/targetSdk 34 rejects). iOS is best-effort only — the lib wraps `beginBackgroundTask`, which grants a short (~30s) window, so a long-lived background server is Android-only.
**Suggested destination**: a new docs/Expo how-to (or wildflower-expo runtime notes)

## Effect spans only emit if the running fiber carries the tracer; `webTelemetryLayerFromEnv()` runs eager globals even when disabled

**Discovered during**: ruthmarks/collector-otel-spans — issue #137 (Collector OTel instrumentation)
**Learning**: `Effect.withSpan` only produces an OTel span when the _running fiber_ has the tracer set in its FiberRef (`@effect/opentelemetry`'s `OtelEffectTracer.layerGlobal` sets it). Code that runs inside a `forkScoped`/`forkDaemon` fiber — e.g. the effect-messaging transport's inbound-dispatch fiber and the collector step-timers — inherits FiberRefs _as a snapshot taken at fork time_, so the span calls in those handlers are silent no-ops unless the tracer was already in context when the fork happened. Fix: provide the telemetry layer to the transport-build effect at boot (`apps/wildflower-react/src/bridges/build-transport.ts` → `Effect.provide(webTelemetryLayerFromEnv())`); the forked fibers then snapshot it. You do NOT need the effect to _require_ the tracer in its `R` — providing the layer sets the FiberRef as a side effect, and the snapshot survives even after the provide-scope closes (FiberRefs are per-fiber copies). Second gotcha, the one that bit the unit test: `webTelemetryLayerFromEnv()` is NOT pure even on the disabled path — `makeClientTelemetryLayer` calls `initClientTelemetry` eagerly at layer-_construction_ time, which installs a global OTel `StackContextManager` and inits Sentry _before_ the `isTelemetryEnabled` check returns `Layer.empty`. So a focused unit test that drives `buildTransport` with everything else mocked must also `vi.mock('telemetry-web', () => ({ ...actual, webTelemetryLayerFromEnv: () => Layer.empty }))` — otherwise those global side effects perturb the test's microtask timing and the shared OTel/Sentry globals leak across tests (observed as a `signalReady` timeout + a doubled console-interceptor call). `initClientTelemetry` is idempotent (`if (registered !== undefined) return registered`), so calling it from `buildTransport` in addition to the app-boot `initWebTelemetryFromEnv()` is safe in production.
**Suggested destination**: docs/Effect/Patterns Reference.md (tracer-in-FiberRef propagation) + docs/Testing/Unit Testing How-To.md (mock telemetry-web in focused tests)

## SPA-fallback 200s make missing API routes surface as `ParseError: Could not parse JSON` in HttpApiClient

**Discovered during**: ruthmarks/add-gatekeeper-rust — debugging the Tauri webview's `/_auth/home/` crash
**Learning**: When an axum (or any) server uses a catch-all SPA fallback (`Router::fallback(serve_spa_fallback)`), a request to an API route the server doesn't implement returns `index.html` with status **200** — so Effect's `HttpApiClient` matches the success status, tries to `JSON.parse` an HTML document, and fails with `ParseError: ... Transformation process failure └─ Could not parse JSON` on the endpoint's success schema. The schema chain in the error (`((unknown <-> string) <-> (string <-> unknown)) <-> <SuccessSchema>`) is the client's body-text→JSON step — recognize that shape as "the server sent a 200 non-JSON body", almost always the SPA fallback swallowing an unimplemented route, NOT a schema/codec drift. Concrete case: the Tauri host's Rust server (`apps/wildflower-tauri/src-tauri/src/lib.rs`) only served gatekeeper + FHIR; `GET /apps`, `/tunnel`, `/collector/remotes` (the app-shell surface implemented only in TS `wildflower-server`) fell through to the fallback. Diagnosis one-liner: `curl -i http://127.0.0.1:8080/<route>` → `200` + HTML confirms it. Mind wire prefixes when stubbing: `ListRemotes` is `/collector/remotes` (the group has `.prefix('/collector')`), not `/remotes`.
**Suggested destination**: docs/Effect/Patterns Reference.md (HttpApiClient failure-mode note)

## `HttpApiSchema.withEncoding` on a payload `Schema.Union` is honored by the server but ignored by the client encoder — and hoisting it into a const poisons the API's `R` channel

**Discovered during**: ruthmarks/add-gatekeeper-rust — webview device-flow `POST /oauth/token` rejected by gatekeeper-rust with `400 invalid_request "Malformed payload"`
**Learning**: Two distinct traps in one schema. (1) Piping `HttpApiSchema.withEncoding({ kind: 'UrlParams', ... })` onto a `Schema.Union(...)` payload wrapper makes `HttpApiBuilder` (server) decode form-urlencoded correctly, but `HttpApiClient` (client) resolves payload encodings **per union member** — the wrapper-level annotation is silently ignored and requests go out as `application/json`. TS↔TS this is invisible (the lenient Effect server still accepts it); a strict RFC 6749 server (gatekeeper-rust parses with `serde_urlencoded` only) rejects the JSON. Fix: pipe `withEncoding` onto **each union member**, leave the union bare. Pinned by `gatekeeper-core/src/http-api-definition/oauth.test.ts` (a capturing-`HttpClient` test that asserts the request body's content type + raw text — write that test BEFORE the fix to confirm the diagnosis). (2) Don't hoist the annotation into a shared const (`const enc = HttpApiSchema.withEncoding({...}); schema.pipe(enc)`): `withEncoding` is generic over the schema it's applied to, and a hoisted const pins those type parameters to `unknown`, which leaks `R = unknown` into the whole `HttpApi`'s requirements channel — surfacing as baffling `Layer<..., never, unknown> is not assignable to Layer<..., never, never>` errors in distant server/client wiring. Write the `.pipe(HttpApiSchema.withEncoding({...}))` call out at each use site.
**Suggested destination**: docs/Effect/Patterns Reference.md (HttpApi payload encoding section)
...
## Node 26's global `localStorage` shadows jsdom's in Vitest — `window.localStorage` reads back `undefined`

**Discovered during**: consolidate-deps-chores — moving the toolchain to Node 26 (PR #289)
**Learning**: Node 22.4+ added WHATWG Web Storage (`localStorage`/`sessionStorage`) as globals, and **Node 26 exposes them by default** — but inert: they read back `undefined` and log `ExperimentalWarning: localStorage is not available because --localstorage-file was not provided` unless you pass `--localstorage-file`. Vitest's jsdom environment populates the test global from a jsdom window but **skips keys that already exist on `globalThis`**, so on Node 26 it does NOT install jsdom's working `localStorage`; `window.localStorage` (=== the global) resolves to Node's inert one and `window.localStorage.clear()` throws `TypeError: Cannot read properties of undefined (reading 'clear')`. Passes on Node 22/24 (no global present), fails only under Node 26 — so it surfaced in CI's `Lint + Test` (which runs on the newer Node) while `vp test` on an older local Node stayed green. Fix: launch the test workers with `--no-experimental-webstorage`, which removes Node's global entirely so jsdom installs its own. Getting that flag onto the workers is the fiddly part: `poolOptions.execArgv` was **removed in Vitest 4** (pool options are now top-level), and the replacement top-level `test.execArgv` is **ignored on the root projects config** — it only takes effect when set on an individual project's own config. So the flag lives in `vite.config.base.ts`'s `test` block (`execArgv: ['--no-experimental-webstorage']`) and every jsdom package opts in by spreading it: `test: { ...base.test, environment: 'jsdom', … }` (the same `...base.<key>` pattern the base already documents for `resolve`/`lint`). This covers a bare `vp test` too, unlike a `NODE_OPTIONS=…` prefix on the npm scripts (which misses direct `vp test` invocations). The flag is accepted on Node 22.4+, so it's safe across every supported runtime; a new jsdom package must remember to spread `...base.test`.
**Suggested destination**: docs/Testing/ (a "Node runtime gotchas" note) or the Vite+ block in AGENTS.md
