...
if [[ -z "$profile_app_id" ]]; then
  fail "IOS_MOBILE_PROVISION decodes as a valid profile but carries no application-identifier entitlement this check can read. That is a bug in this script or a profile shape it does not know, not a reason to regenerate the profile."
fi
